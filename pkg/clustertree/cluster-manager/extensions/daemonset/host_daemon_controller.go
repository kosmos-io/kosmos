...
// The host_daemon_controller is developed based on the Kubernetes daemon_controller.go, mainly to change the reconciled objects from daemonsets to shadowdaemonset.kosmos.io.
// For more detail on daemon_controller.go ref: https://github.com/kubernetes/kubernetes/blob/release-1.25/pkg/controller/daemon/daemon_controller.go
...
// ShadowControllerKind contains the schema.GroupVersionKind for this controller type.
var ShadowControllerKind = kosmosv1alpha1.SchemeGroupVersion.WithKind("ShadowDaemonSet")

// HostDaemonSetsController HostHostDaemonSetsController is responsible for synchronizing ShadowDaemonSet objects stored
...
func NewHostDaemonSetsController(
	...
	dsc := &HostDaemonSetsController{
		...
		eventRecorder:    eventBroadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: "host-daemonset-controller"}),
		podControl: controller.RealPodControl{
			KubeClient: kubeClient,
			Recorder:   eventBroadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: "host-daemonset-controller"}),
		},
	...
	// nolint:errcheck
	daemonSetRefInformer.Informer().AddEventHandler(cache.FilteringResourceEventHandler{
		FilterFunc: func(obj interface{}) bool {
			sds, ok := obj.(*kosmosv1alpha1.ShadowDaemonSet)
			if !ok {
				return false
			}
			return sds.RefType == kosmosv1alpha1.RefTypeHost
		},
...
func (dsc *HostDaemonSetsController) Run(ctx context.Context, workers int) {
	...
	klog.Infof("Starting host_daemon_controller")
	defer klog.Infof("Shutting down host_daemon_controller")

	if !cache.WaitForNamedCacheSync("host_daemon_controller", ctx.Done(), dsc.podStoreSynced, dsc.nodeStoreSynced, dsc.historyStoreSynced, dsc.dsStoreSynced) {
		return
	}
...
func (dsc *HostDaemonSetsController) getDaemonPods(ctx context.Context, ds *kosmosv1alpha1.ShadowDaemonSet) ([]*v1.Pod, error) {
	...
	cm := controller.NewPodControllerRefManager(dsc.podControl, ds, selector, ShadowControllerKind, dsNotDeleted)
...
func (dsc *HostDaemonSetsController) resolveControllerRef(namespace string, controllerRef *metav1.OwnerReference) *kosmosv1alpha1.ShadowDaemonSet {
	...
	if controllerRef.Kind != ShadowControllerKind.Kind {
		return nil
	}
		...
		for i := pos; i < pos+batchSize; i++ {
			go func(ix int) {
				...
				err := dsc.podControl.CreatePods(ctx, ds.Namespace, podTemplate,
					ds, metav1.NewControllerRef(ds, ShadowControllerKind))
...
func storeDaemonSetStatus(
	...
	for i := 0; ; i++ {
		...
		if _, updateErr = kosmosClient.KosmosV1alpha1().ShadowDaemonSets(toUpdate.Namespace).UpdateStatus(ctx, toUpdate, metav1.UpdateOptions{}); updateErr == nil {
			return nil
		}
