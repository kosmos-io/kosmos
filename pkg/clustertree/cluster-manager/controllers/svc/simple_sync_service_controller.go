...
func (c *SimpleSyncServiceController) cleanUpServiceInLeafCluster(namespace string, name string) error {
	...
	for _, cluster := range clusters {
		...
		err = leafClient.Clientset.CoreV1().Services(namespace).Delete(context.TODO(), name, metav1.DeleteOptions{})
...
func (c *SimpleSyncServiceController) syncServiceInLeafCluster(service *corev1.Service) error {
	...
	for _, cluster := range clusters {
		...
		go func(cluster string) {
			...
			if err = c.createNamespace(leafManager.Client, service.Namespace); err != nil && !apierrors.IsAlreadyExists(err) {
			...
			err = c.createOrUpdateServiceInClient(clientService, leafManager)
			if err != nil {
...
func (c *SimpleSyncServiceController) checkServiceType(service *corev1.Service, resource *clustertreeutils.LeafResource) error {
	if *service.Spec.IPFamilyPolicy == corev1.IPFamilyPolicySingleStack {
		if service.Spec.IPFamilies[0] == corev1.IPv6Protocol && resource.IPFamilyType == kosmosv1alpha1.IPFamilyTypeIPV4 ||
			...
			return fmt.Errorf("service's IPFamilyPolicy %s is not match the leaf cluster %s", *service.Spec.IPFamilyPolicy, resource.ClusterName)
		}
	}
	return nil
}
...
func (c *SimpleSyncServiceController) createOrUpdateServiceInClient(service *corev1.Service, leafManger *clustertreeutils.LeafResource) error {
	...
	if err != nil {
		...
		klog.Errorf("Get service(%s/%s) from in cluster %s failed, Error: %v", service.Namespace, service.Name, leafManger.ClusterName, err)
		return err
	}

	retainServiceFields(oldService, service)

	if err := leafManger.Client.Update(context.TODO(), service); err != nil {
		if err != nil {
			klog.Errorf("Update serviceImport service(%s/%s) in cluster %s failed, Error: %v", service.Namespace, service.Name, leafManger.ClusterName, err)
			return err
		}
	}
	return nil
