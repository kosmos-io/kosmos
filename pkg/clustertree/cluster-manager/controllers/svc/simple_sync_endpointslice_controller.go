...
func (c *SimpleSyncEPSController) SetupWithManager(mgr manager.Manager) error {
	epsPredicate := builder.WithPredicates(predicate.Funcs{
		...
		UpdateFunc: func(updateEvent event.UpdateEvent) bool {
			...
			oldEps, ok := updateEvent.ObjectOld.(*discoveryv1.EndpointSlice)
			if !ok {
				return false
			}

			return c.shouldEnqueue(newEps) != c.shouldEnqueue(oldEps)
		},
...
func (c *SimpleSyncEPSController) cleanUpEpsInLeafCluster(namespace string, name string) error {
	...
	for _, cluster := range clusters {
		...
		err = leafClient.Clientset.DiscoveryV1().EndpointSlices(namespace).Delete(context.TODO(), name, metav1.DeleteOptions{})
...
func (c *SimpleSyncEPSController) syncEpsInLeafCluster(eps *discoveryv1.EndpointSlice, serviceName string) error {
	...
	for _, cluster := range clusters {
		...
		go func(cluster, serviceName string) {
			...
			if err = c.createNamespace(leafManager.Client, endpointSlice.Namespace); err != nil && !apierrors.IsAlreadyExists(err) {
			...
			if err = leafManager.Client.Create(context.TODO(), newSlice); err != nil {
				if apierrors.IsAlreadyExists(err) {
					if err = c.updateEndpointSlice(newSlice, leafManager); err != nil {
...
func (c *SimpleSyncEPSController) updateEndpointSlice(slice *discoveryv1.EndpointSlice, leafManager *clustertreeutils.LeafResource) error {
	eps := slice.DeepCopy()
