...
// 观察pod的变化，如果rootpod的Status异常就触发一次leadpod的状态同步到rootpod同步
...
func (c *PodSyncController) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	//pod都在相同的NS？如果 request.NamespacedName 中有明确的信息指向某个特定的 leaf cluster，可以直接查询该集群，避免遍历。
	...
	for _, cluster := range clusters {
		...
		// 对 leafPod 的状态变化处理较为机械,对比了 rootPod.Status 和 leafPod.Status，但没有明确判断状态同步的必要性。例如，如果状态变化不涉及重要字段，可能没有必要同步。
