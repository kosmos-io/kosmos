...
func (h *helper) Refresh() error {
	...
	serverPreferredResources, err := refreshServerPreferredResources(h.discoveryClient)
	if err != nil {
		return errors.WithStack(err)
	}
	...
	shortcutExpander, err := utils.NewShortcutExpander(restmapper.NewDiscoveryRESTMapper(groupResources), h.resources)
	if err != nil {
		return errors.WithStack(err)
	}
...
func refreshServerPreferredResources(discoveryClient serverResourcesInterface) ([]*metav1.APIResourceList, error) {
	preferredResources, err := discoveryClient.ServerPreferredResources()
	if err != nil {
		if discoveryErr, ok := err.(*discovery.ErrGroupDiscoveryFailed); ok {
			for groupVersion, err := range discoveryErr.Groups {
				klog.Warningf("Failed to discover group: %v. %v", groupVersion, err)
			}
