...
type IncludesExcludes struct {
	...
	excludes sets.Set[string]
}

func NewIncludesExcludes() *IncludesExcludes {
	return &IncludesExcludes{
		includes: sets.KeySet(make(map[string]struct{})),
		excludes: sets.KeySet(make(map[string]struct{})),
	}
}
...
func (ie *GlobalIncludesExcludes) ShouldInclude(typeName string) bool {
	...
	if err != nil {
		klog.Errorf("fail to get resource %s. %s", typeName, err.Error())
		return false
	}

	if !resource.Namespaced && IsSetToFalse(ie.includeClusterResources) {
		klog.Info("Skipping resource %s, because it's cluster-scoped, and IncludeClusterResources is set to false.", typeName)
		return false
	}
	...
	if typeName != kuberesource.Namespaces.String() && !resource.Namespaced &&
		...
		klog.Infof("Skipping resource %s, because it's cluster-scoped and only specific namespaces or namespace scope types are included in the backup.", typeName)
		return false
	}
...
func (ie *GlobalIncludesExcludes) ShouldExclude(typeName string) bool {
	...
	if ie.resourceFilter.excludes.Has(typeName) {
		return true
	}
	...
	if !resource.Namespaced {
		...
		if IsSetToFalse(ie.includeClusterResources) {
			return true
		}
...
func (ie *ScopeIncludesExcludes) ShouldInclude(typeName string) bool {
	...
	if resource.Namespaced {
		if ie.namespaceScopedResourceFilter.excludes.Has("*") || ie.namespaceScopedResourceFilter.excludes.Has(typeName) {
			return false
		}

		// len=0 means include everything
		return ie.namespaceScopedResourceFilter.includes.Len() == 0 || ie.namespaceScopedResourceFilter.includes.Has("*") || ie.namespaceScopedResourceFilter.includes.Has(typeName)
	}

	if ie.clusterScopedResourceFilter.excludes.Has("*") || ie.clusterScopedResourceFilter.excludes.Has(typeName) {
		return false
	}
	...
	if ie.clusterScopedResourceFilter.includes.Len() == 0 &&
		ie.clusterScopedResourceFilter.excludes.Len() == 0 &&
	...
	return ie.clusterScopedResourceFilter.includes.Has("*") || ie.clusterScopedResourceFilter.includes.Has(typeName) || typeName == kuberesource.Namespaces.String()
}
...
func (ie *ScopeIncludesExcludes) ShouldExclude(typeName string) bool {
	...
	if resource.Namespaced {
		if ie.namespaceScopedResourceFilter.excludes.Has(typeName) {
			return true
		}
	} else {
		if ie.clusterScopedResourceFilter.excludes.Has(typeName) {
			return true
		}
	}
	return false
...
func (ie *IncludesExcludes) ExcludesString() string {
	return asString(ie.excludes, "<none>")
}

func asString(in sets.Set[string], empty string) string {
	if in.Len() == 0 {
		return empty
	}
	resultSlice := []string{}
...
func newScopeIncludesExcludes(nsIncludesExcludes IncludesExcludes, helper discovery.Helper) *ScopeIncludesExcludes {
...
func generateScopedIncludesExcludes(namespacedIncludes, namespacedExcludes, clusterIncludes, clusterExcludes []string, mapFunc func(string, bool) string, nsIncludesExcludes IncludesExcludes, helper discovery.Helper) *ScopeIncludesExcludes {
	res := newScopeIncludesExcludes(nsIncludesExcludes, helper)
...
func generateFilter(filter sets.Set[string], resources []string, mapFunc func(string, bool) string, namespaced bool) {
...
func GetGlobalResourceIncludesExcludes(helper discovery.Helper, includes, excludes []string, includeClusterResources *bool, nsIncludesExcludes IncludesExcludes) *GlobalIncludesExcludes {
	...
	klog.Infof("Including resources: %s", ret.resourceFilter.IncludesString())
	klog.Infof("Excluding resources: %s", ret.resourceFilter.ExcludesString())
	return ret
}
...
func GetScopeResourceIncludesExcludes(helper discovery.Helper, namespaceIncludes, namespaceExcludes, clusterIncludes, clusterExcludes []string, nsIncludesExcludes IncludesExcludes) *ScopeIncludesExcludes {
	...
	klog.Infof("Including namespace-scoped resources: %s", ret.namespaceScopedResourceFilter.IncludesString())
	klog.Infof("Excluding namespace-scoped resources: %s", ret.namespaceScopedResourceFilter.ExcludesString())
	klog.Infof("Including cluster-scoped resources: %s", ret.clusterScopedResourceFilter.IncludesString())
	klog.Infof("Excluding cluster-scoped resources: %s", ret.clusterScopedResourceFilter.ExcludesString())
