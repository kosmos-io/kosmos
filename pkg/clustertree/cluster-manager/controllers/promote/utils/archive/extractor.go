...
func NewExtractor(fs filesystem.Interface) *Extractor {
	return &Extractor{
		fs: fs,
	}
}
...
func (e *Extractor) UnzipAndExtractBackup(src io.Reader) (string, error) {
	...
	if err != nil {
		klog.Infof("error creating gzip reader: %v", err)
		return "", err
	}
	defer gzr.Close()
...
func (e *Extractor) readBackup(tarRdr *tar.Reader) (string, error) {
	...
	if err != nil {
		klog.Infof("error creating temp dir: %v", err)
		return "", err
	}
	...
	for {
		...
		if err != nil {
			klog.Infof("error reading tar: %v", err)
			return "", err
		}
		...
		case tar.TypeDir:
			...
			if err != nil {
				klog.Infof("mkdirall error: %v", err)
				return "", err
			}
		...
		case tar.TypeReg:
			...
			if err := e.writeFile(target, tarRdr); err != nil {
				klog.Infof("error copying: %v", err)
				return "", err
			}
		}
	}
