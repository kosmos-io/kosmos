...
func (p *Parser) Parse(dir string) (map[string]*ResourceItems, error) {
	...
	resourcesDir := filepath.Join(dir, constants.ResourcesDir)
	...
	for _, resourceDir := range resourceDirs {
		if !resourceDir.IsDir() {
			klog.Warningf("Ignoring unexpected file %q in directory %q", resourceDir.Name(), strings.TrimPrefix(resourcesDir, dir+"/"))
			continue
		}
		...
		clusterScopedDir := filepath.Join(resourcesDir, resourceDir.Name(), constants.ClusterScopedDir)
		...
		namespaceScopedDir := filepath.Join(resourcesDir, resourceDir.Name(), constants.NamespaceScopedDir)
		...
		if exists {
			...
			for _, namespaceDir := range namespaceDirs {
				if !namespaceDir.IsDir() {
					klog.Warningf("Ignoring unexpected file %q in directory %q", namespaceDir.Name(), strings.TrimPrefix(namespaceScopedDir, dir+"/"))
					continue
				}
...
func (p *Parser) getResourceItemsForScope(dir, archiveRootDir string) ([]string, error) {
	...
	for _, file := range files {
		if file.IsDir() {
			klog.Warningf("Ignoring unexpected subdirectory %q in directory %q", file.Name(), strings.TrimPrefix(dir, archiveRootDir+"/"))
			continue
		}
...
func (p *Parser) ParseGroupVersions(dir string) (map[string]metav1.APIGroup, error) {
	...
	for _, rgd := range rgDirs {
		...
		for _, gvd := range gvDirs {
			...
			if strings.Contains(gvdName, constants.PreferredVersionDir) {
