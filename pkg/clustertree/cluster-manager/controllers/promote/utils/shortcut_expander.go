...
func NewShortcutExpander(delegate meta.RESTMapper, resources []*metav1.APIResourceList) (shortcutExpander, error) {
	return shortcutExpander{RESTMapper: delegate, resources: resources}, nil
}
	...
	for _, apiResources := range e.resources {
		for _, apiRes := range apiResources.APIResources {
			for _, shortName := range apiRes.ShortNames {
				...
				if err != nil {
					klog.Errorf("Unable to parse groupversion %s. %v", apiResources.GroupVersion, err)
					continue
				}
