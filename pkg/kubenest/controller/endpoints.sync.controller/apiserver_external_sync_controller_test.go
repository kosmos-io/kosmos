...
// TestSyncAPIServerExternalEndpoints tests the SyncAPIServerExternalEndpoints function
func TestSyncAPIServerExternalEndpoints(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := controller.SyncAPIServerExternalEndpoints(ctx, fakeHostClusterClient, vc)
			...
			if tt.wantErr {
				...
				if tt.wantSubsets != nil {
					createdEndpoint, err := fakeHostClusterClient.CoreV1().Endpoints(constants.KosmosNs).Get(ctx, constants.APIServerExternalService, metav1.GetOptions{})
					assert.NoError(t, err)
