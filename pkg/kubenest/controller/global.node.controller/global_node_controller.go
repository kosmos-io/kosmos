...
// sync node status to globalnode,if STATE=="free" or "reserved",directly update
// else get virtualcluster kubeconfig and use its node to update
func (r *GlobalNodeController) SyncNodeStatus(ctx context.Context, globalNode *v1alpha1.GlobalNode) error {
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		klog.V(4).Infof("global-node-controller: syncNodeStatus: successfully updated global node %s", updateGlobalNode.Name)
		return nil
	})
	return err
}
