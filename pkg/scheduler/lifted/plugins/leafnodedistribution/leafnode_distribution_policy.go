...
import (
	...
	kosmosutil "github.com/kosmos.io/kosmos/pkg/utils"
)
...
func initDistributionInformer(kubeConfig *restclient.Config) (*versioned.Clientset, *dpLister.DistributionPolicyLister, *dpLister.ClusterDistributionPolicyLister, error) {
...
func New(_ runtime.Object, fh framework.Handle) (framework.Plugin, error) {
	...
	client, dpLister, cdpLister, err := initDistributionInformer(fh.KubeConfig())
	if err != nil {
...
func (lnd *LeafNodeDistribution) Filter(ctx context.Context, state *framework.CycleState, pod *corev1.Pod, nodeInfo *framework.NodeInfo) *framework.Status {
	...
	case kosmosv1alpha1.HOSTNODE:
		{
			if _, ok := nodeLabels[kosmosutil.KosmosNodeLabel]; ok {
