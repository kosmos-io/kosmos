...
func (o *BackoffOptions) AddFlags(fs *pflag.FlagSet) {
	fs.DurationVar(&o.Duration, "retry-duration", 5*time.Millisecond, "the retry duration.")
	fs.Float64Var(&o.Factor, "retry-factor", 1.0, "Duration is multiplied by factor each iteration.")
	fs.Float64Var(&o.Jitter, "retry-jitter", 0.1, "The sleep at each iteration is the duration plus an additional amount.")
	fs.IntVar(&o.Steps, "retry-steps", 5, "The retry steps when update resource.")
}
...
func DefaultUpdateRetryBackoff(opts BackoffOptions) wait.Backoff {
	// set defaults
	if opts.Duration <= 0 {
		opts.Duration = 5 * time.Millisecond
	}
	if opts.Factor <= 0 {
		opts.Factor = 1.0
	}
