...
func TestFdbs(t *testing.T) {
	t.Run("test create and load fdb", func(t *testing.T) {
		...
		record := &clusterlinkv1alpha1.Fdb{
			IP:  "220.76.1.4",
			Mac: "a6:01:63:1f:cd:a2",
			Dev: "vx-bridge",
		}

		if err := addFdb(*record); err != nil {
			t.Error(err)
			return
		}

		records, err := loadFdbs()
		...
		if err := deleteFdb(*record); err != nil {
			t.Error(err)
			return
		}

		deletes, delerr := loadFdbs()
