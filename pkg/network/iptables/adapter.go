...
// For reference:
// https://github.com/submariner-io/submariner/blob/v0.15.1/pkg/iptables/adapter.go
...
func (a *Adapter) InsertUnique(table, chain string, position int, ruleSpec []string) error {
	...
	for index, rule := range rules {
		if strings.Contains(rule, strings.Join(ruleSpec, " ")) {
			klog.Infof("In %s table, iptables rule \"%s\", exists at index %d.", table, strings.Join(ruleSpec, " "), index)
			numOccurrences++
	...
	if numOccurrences == 1 && isPresentAtRequiredPosition {
		klog.Infof("In %s table, iptables rule \"%s\", already exists.", table, strings.Join(ruleSpec, " "))
		return nil
...
func (a *Adapter) UpdateChainRules(table, chain string, rules [][]string) error {
	...
	for _, ruleSpec := range rules {
		...
		if ruleStrings.Has(ruleString) {
			...
			klog.Infof("Adding iptables rule in %q, %q: %q", table, chain, ruleSpec)
	...
	for _, rule := range ruleStrings.UnsortedList() {
		klog.Infof("Deleting stale iptables rule in %q, %q: %q", table, chain, rule)
