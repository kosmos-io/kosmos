...
func RegisterDefaults(scheme *runtime.Scheme) error {
	...
	scheme.AddTypeDefaultingFunc(&ShadowDaemonSet{}, func(obj interface{}) { SetObjectDefaults_ShadowDaemonSet(obj.(*ShadowDaemonSet)) })
	scheme.AddTypeDefaultingFunc(&ShadowDaemonSetList{}, func(obj interface{}) { SetObjectDefaults_ShadowDaemonSetList(obj.(*ShadowDaemonSetList)) })
	return nil
}
...
func SetObjectDefaults_ShadowDaemonSet(in *ShadowDaemonSet) {
	SetDefaults_ShadowDaemonSet(in)
	for i := range in.DaemonSetSpec.Template.Spec.InitContainers {
		a := &in.DaemonSetSpec.Template.Spec.InitContainers[i]
		for j := range a.Ports {
			b := &a.Ports[j]
			if b.Protocol == "" {
				b.Protocol = "TCP"
	...
	for i := range in.DaemonSetSpec.Template.Spec.Containers {
		a := &in.DaemonSetSpec.Template.Spec.Containers[i]
		for j := range a.Ports {
			b := &a.Ports[j]
			if b.Protocol == "" {
				b.Protocol = "TCP"
	...
	for i := range in.DaemonSetSpec.Template.Spec.EphemeralContainers {
		a := &in.DaemonSetSpec.Template.Spec.EphemeralContainers[i]
		for j := range a.EphemeralContainerCommon.Ports {
			b := &a.EphemeralContainerCommon.Ports[j]
			if b.Protocol == "" {
				b.Protocol = "TCP"
...
func SetObjectDefaults_ShadowDaemonSetList(in *ShadowDaemonSetList) {
	for i := range in.Items {
		a := &in.Items[i]
		SetObjectDefaults_ShadowDaemonSet(a)
	}
}
